...
export default class extends Base {
    ...
    async newProjectAction() {
        ...
        pro.status = 1;
        pro.type = this.param('type');
        pro.dir = this.param('dir');
        pro.machine_id = this.param('machine_id')
        pro.sdir = this.param('sdir');
        pro.task = this.param('task')
        pro.server_dir = this.param('server_dir')
        pro.after_deploy_shell = this.param('after_deploy_shell')
        pro.before_deploy_shell = this.param('before_deploy_shell')
    ...
    async getBuildResultAction() {
        ...
        let fDir = buildDir.replace('./', '') + dir;
        let pathInZip = '';
        if (isZip == "true") {
            let zipFileArr = dir.split('.zip');
            fDir = buildDir.replace('./', '') + zipFileArr[0] + '.zip';
            pathInZip = zipFileArr[1];
        } else {
            fDir = buildDir.replace('./', '') + dir;
        }
    ...
    async doDeploy(proId, machineId, incExc, deployDesc, deployPass, deployFiles, machineArr, onlineReason, notifyCharger, codeUrl, vcsType, lastTag) {
        ...
        for (let i = 0; i < machineArr.length; i++) {
            ...
            let sourceDirList = bInfo.sdir.split(';');
            let targetDirList = bInfo.dir.split(';');
            deployInfo.pro_id = bInfo.id;
            deployInfo.pro_name = bInfo.name;
            deployInfo.sessionUser = sessionUser;
            let server_dir = bInfo.server_dir;
            ...
            let beforeShell = bInfo.before_deploy_shell;
            ...
            let afterShell = bInfo.after_deploy_shell;
            //远程调用部署后脚本
